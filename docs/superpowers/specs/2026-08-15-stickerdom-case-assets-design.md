...
- **Prefab'lar:** `Prefabs/Sticker/` beşlisi: `CardModel`, `SheetModel`, `DrawPileModel`, `WastePileModel`, `PegModel` (+ meta, GUID korunarak). 5 şirket Model scripti sökülür. Font: keşifte Feel bundle'ındaki LiberationSans SDF'in GUID'inin TMP Essentials'la AYNI olduğu doğrulandı (`8f586378b4e144a9851e7b34d9b748ee`) — remap GEREKMEZ; referanslar repodaki mevcut `Assets/TextMesh Pro` kopyasına kendiliğinden çözülür. Tek kural: Feel'in kopyası hedefe KOPYALANMAZ (GUID çakışması — ortak-şablon dersi).
